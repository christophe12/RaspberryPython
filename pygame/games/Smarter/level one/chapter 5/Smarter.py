...
USERSCORE = 0 # holds the points of the user
GAMESCORERECT = None # holds the game score_rect obj
...
def drawWinningScore():
	""" This function will draw the main score for passing the level to the screen"""
	global GAMESCORERECT
	# Score text title
	scoreObj = pygame.font.SysFont('verdana', 18)
	scoreSurfaceObj = scoreObj.render('Score', True, WHITE)
	score_text_rect = scoreSurfaceObj.get_rect()
	scorex = WINDOWWIDTH - (score_text_rect.width + 30)
	scorey = WINDOWHEIGHT - 370
	...
	GAMESCORERECT = score_rect #passing the score_rect properties
    # the score number inside the rectangle
	score_number = calculateQuestionsScore()
	score_number_obj = pygame.font.SysFont('verdana', 18)
	score_number_surface_obj = score_number_obj.render(score_number, True, WHITE)
	...
	DISPLAYSURF.blit(scoreSurfaceObj, (scorex, scorey))
	pygame.draw.rect(DISPLAYSURF, WHITE, score_rect, 1)
	DISPLAYSURF.blit(score_number_surface_obj, (score_number_x, score_number_y))

def setUserScore(user_score):
	""" This function will set the userscore."""
	global USERSCORE
	USERSCORE += user_score
	return USERSCORE

def getUserScore():
	""" This function will give us the USERSCORE"""
	return USERSCORE

def removeScore(score_text):
	global USER_SCORE_RECT
	pygame.draw.rect(DISPLAYSURF, BACKGROUNDCOLOR, USER_SCORE_RECT)
	pygame.display.update()


def drawUserScore():
	""" This function will draw and update the userscore on the main window."""
	global USERSCORE, USER_SCORE_RECT

	game_score_rect = GAMESCORERECT
	...
	pygame.draw.rect(DISPLAYSURF, WHITE, user_score_rect, 1)

	if USERSCORE == 0:
		user_score = str(getUserScore())
		user_score_text_obj = pygame.font.SysFont('verdana', 18)
		user_score_text_surface_obj = user_score_text_obj.render(user_score, True, WHITE)
		...
		USER_SCORE_RECT = pygame.Rect(user_score_text_x, user_score_text_y, user_score_text_rect.width, user_score_text_rect.height)
		DISPLAYSURF.blit(user_score_text_surface_obj, (user_score_text_x, user_score_text_y))
	else:
		removeScore(USER_SCORE_RECT)
		user_score = str(getUserScore())
		user_score_text_obj = pygame.font.SysFont('verdana', 18)
		user_score_text_surface_obj = user_score_text_obj.render(user_score, True, WHITE)
		user_score_text_rect = user_score_text_surface_obj.get_rect()
...
def getQuestionScore(question_index):
	""" This function will return the current question score in the game loop."""
	return int(data[question_index]['score'])
		...
		if event.type == QUIT:
			...
			if COVERED:
				if checkAnswer(COVEREDTEXT):
					...
					question_score = getQuestionScore(STATE)
					setUserScore(question_score)
					drawUserScore()
